import type { Metadata } from "next";
import AdminPageHeader from "@/app/components/admin/ui/AdminPageHeader";
import ComponentsTabs from "@/app/components/admin/layout/ComponentsTabs";
import { ADMIN_BASE } from "@/lib/admin/constants";
import { getAdminSession } from "@/lib/admin/require-admin";

export const metadata: Metadata = { title: "Bileşenler" };
export const dynamic = "force-dynamic";

export default async function AdminComponentsPage() {
  const session = await getAdminSession();
  const isAdmin = session?.role === "ADMIN";

  return (
    <div>
      <AdminPageHeader
        title="Bileşenler"
        description="Footer, menü paneli ve iletişim formu metinlerini dil bazında düzenleyin."
        breadcrumb={[
          { label: "Admin", href: ADMIN_BASE },
          { label: "Bileşenler" },
        ]}
      />
      {isAdmin ? (
        <ComponentsTabs />
      ) : (
        <div className="rounded-2xl border border-dashed border-black/15 bg-white px-6 py-16 text-center">
          <h2 className="text-lg font-semibold text-px-black">Yalnızca admin</h2>
          <p className="mt-2 text-sm text-px-body">
            Layout bileşenlerini görüntülemek ve düzenlemek için ADMIN rolü gerekir.
          </p>
        </div>
      )}
    </div>
  );
}
